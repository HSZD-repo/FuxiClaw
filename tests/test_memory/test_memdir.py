...
def test_structured_memory_entry_is_searchable_by_keywords(tmp_path: Path, monkeypatch):
    monkeypatch.setenv("OPENHARNESS_DATA_DIR", str(tmp_path / "data"))
    project_dir = tmp_path / "repo"
    project_dir.mkdir()
    ...
    path = add_structured_memory_entry(
        project_dir,
        title="Conda unavailable in bio container",
        content="Do not run conda install in the bioinformatics Docker container. Use the prebuilt image.",
        memory_type="known_failure",
        scope="docker",
        keywords=["conda", "scanpy", "docker"],
        priority="high",
    )

    assert path.name == "conda_unavailable_in_bio_container.md"
    matches = find_relevant_memories("install scanpy with conda", project_dir)
    assert matches
    assert matches[0].path.name == path.name
    assert matches[0].memory_type == "known_failure"
    assert matches[0].priority == "high"
    assert "scanpy" in matches[0].keywords
