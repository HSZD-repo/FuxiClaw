...
def test_list_session_snapshots_filters_empty_sessions(tmp_path: Path, monkeypatch):
    monkeypatch.setenv("OPENHARNESS_DATA_DIR", str(tmp_path / "data"))
    project = tmp_path / "repo"
    project.mkdir()
    ...
    save_session_snapshot(
        ...
        model="claude-test",
        system_prompt="system",
        messages=[ConversationMessage(role="user", content=[TextBlock(text="real session")])],
        usage=UsageSnapshot(),
        session_id="real123",
    )
    ...
    assert [session["session_id"] for session in sessions] == ["real123"]
    assert sessions[0]["summary"] == "real session"


def test_rename_session_snapshot_persists_manual_summary(tmp_path: Path, monkeypatch):
    monkeypatch.setenv("OPENHARNESS_DATA_DIR", str(tmp_path / "data"))
    project = tmp_path / "repo"
    project.mkdir()
    ...
    save_session_snapshot(
        ...
        model="claude-test",
        system_prompt="system",
        messages=[ConversationMessage(role="user", content=[TextBlock(text="original user prompt")])],
        usage=UsageSnapshot(),
        session_id="rename123",
    )

    assert rename_session_snapshot(project, "rename123", "  My renamed chat  ") is True
    sessions = list_session_snapshots(project)
    assert sessions[0]["summary"] == "My renamed chat"
    ...
    save_session_snapshot(
        ...
        system_prompt="system",
        messages=[
            ConversationMessage(role="user", content=[TextBlock(text="original user prompt")]),
    ...
    snapshot = load_session_snapshot(project)
    assert snapshot is not None
    assert snapshot["summary"] == "My renamed chat"
    assert snapshot["summary_overridden"] is True
