...
@pytest.mark.asyncio
async def test_grep_tool_registers_cancel_handle_for_rg_process(monkeypatch, tmp_path: Path):
    tool = GrepTool()
    monkeypatch.setattr("openharness.tools.grep_tool.shutil.which", lambda _: "/usr/bin/rg")
    fake_process = _FakeProcess(stdout=_ValueErrorThenEofStdout())
    ...
    assert result.is_error is False
    assert registered["tool_name"] == "grep"
    assert registered["tool_use_id"] == "tool-grep"
