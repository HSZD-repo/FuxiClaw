...
from openharness.tools.bash_tool import (
    BashTool,
    BashToolInput,
    _format_output,
    _normalize_git_command,
    _preflight_interactive_command,
)
...
def test_format_output_strips_macos_script_eof_marker():
    assert _format_output(bytearray(b"^D\x08\x08hello\n")) == "hello"


def test_normalize_git_command_disables_pager_for_read_only_views():
    assert _normalize_git_command("git diff --stat") == "GIT_PAGER=cat TERM=dumb git --no-pager diff --stat"


def test_preflight_rejects_interactive_git_patch_command():
    result = _preflight_interactive_command("git add -p")

    assert result is not None
    assert "likely to wait for interactive input" in result
