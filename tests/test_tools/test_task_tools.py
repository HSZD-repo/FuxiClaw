...
@pytest.mark.asyncio
async def test_task_create_registers_cancel_handle_that_stops_created_task(tmp_path: Path, monkeypatch):
    monkeypatch.setenv("OPENHARNESS_DATA_DIR", str(tmp_path / "data"))
    ...
    create_result = await TaskCreateTool().execute(
        TaskCreateToolInput(
            ...
            command="python -u -c \"import time; time.sleep(60)\"",
        ),
        context,
    )
    ...
    assert create_result.is_error is False
    task_id = create_result.output.split()[2]
    assert registered["cancel_kind"] == "background_task"
    assert callable(registered["cancel"])
