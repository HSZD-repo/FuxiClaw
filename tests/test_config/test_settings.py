...
class TestSettings:
    def test_defaults(self):
        ...
        assert s.tool_execution_mode == "serial"
    ...
    def test_openharness_tool_execution_mode_env_override(self, monkeypatch):
        monkeypatch.setenv("OPENHARNESS_TOOL_EXECUTION_MODE", "parallel")
...
class TestModelProviderProfiles:
    def test_default_profiles_include_supported_settings_providers(self):
        from openharness.config.settings import default_provider_profiles

        profiles = default_provider_profiles()

        assert "gemini" not in profiles
        assert profiles["moonshot"].allowed_models == ["kimi-k2.5", "kimi-k2.6"]
        assert profiles["deepseek"].allowed_models == ["deepseek-v4-flash", "deepseek-v4-pro"]
        assert profiles["dashscope"].label == "Alibaba DashScope (Qwen)"
        assert profiles["dashscope"].provider == "dashscope"
        assert profiles["dashscope"].auth_source == "dashscope_api_key"
        assert profiles["dashscope"].default_model == "qwen3.5-flash"
        assert profiles["dashscope"].base_url == "https://dashscope.aliyuncs.com/compatible-mode/v1"
        assert profiles["dashscope"].allowed_models == ["qwen3.5-flash", "qwen3-max"]
        assert profiles["zhipu"].default_model == "glm-5.1"
        assert profiles["mimo"].default_model == "mimo-2.5"
        assert profiles["openai-compatible"].default_model == "gpt-5.5"
        assert "claude-opus-4-7" in profiles["claude-api"].allowed_models

    @pytest.mark.parametrize(
        ("provider", "auth_source", "env_name"),
        [
    ...
    def test_new_provider_auth_sources(
        self,
        provider: str,
        auth_source: str,
        env_name: str,
        ...
        monkeypatch.setenv(env_name, f"{provider}-test-key")

        assert auth_source_provider_name(auth_source) == provider
        assert default_auth_source_for_provider(provider) == auth_source

        settings = Settings(active_profile=provider)
        resolved = settings.resolve_auth()

        assert resolved.value == f"{provider}-test-key"
        assert env_name in resolved.source

    def test_deepseek_ignores_generic_openai_api_key(self, tmp_path: Path, monkeypatch):
        monkeypatch.setenv("OPENHARNESS_CONFIG_DIR", str(tmp_path / "config"))
        monkeypatch.setenv("OPENAI_API_KEY", "wrong-openai-key")
        monkeypatch.delenv("DEEPSEEK_API_KEY", raising=False)

        settings = Settings(active_profile="deepseek")

        with pytest.raises(ValueError, match="deepseek_api_key"):
            settings.resolve_auth()

    def test_stored_provider_key_beats_provider_env_key(self, tmp_path: Path, monkeypatch):
        monkeypatch.setenv("OPENHARNESS_CONFIG_DIR", str(tmp_path / "config"))
        monkeypatch.setenv("DEEPSEEK_API_KEY", "old-env-key")
        store_credential("deepseek", "api_key", "new-ui-key", use_keyring=False)

        settings = Settings(active_profile="deepseek")
        resolved = settings.resolve_auth()

        assert resolved.value == "new-ui-key"
        assert resolved.source == "file:deepseek"
