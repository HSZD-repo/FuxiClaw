...
Bioinformatics tool list: see [`README-bio-tools.md`](README-bio-tools.md).
...
### 5. Web UI (Browser Chat)

Prefer a browser over the terminal? OpenHarness ships a standalone React UI in
[`frontend/application-ui/`](frontend/application-ui/) that talks to a tiny
WebSocket backend in [`src/openharness/web/`](src/openharness/web/). It runs
independently of the Ink TUI, so you can use both side-by-side.

> **Status** — chat round-trip with any OpenAI-compatible model (OpenAI, Kimi/Moonshot,
> GLM, DeepSeek, MiniMax, Ollama, …) works end-to-end with streaming. Tool
> calls, session history, and permission flows are still on the roadmap.

**One-time setup** — create an isolated venv so backend deps stay out of your
global Python, then drop your LLM credentials into `.env`:
...
cp .env.example .env     # then edit .env and fill in OPENAI_API_KEY (+ base URL / model)
```

`.env` is git-ignored; the dev script auto-sources it on startup, so you don't
need to `export` anything in your shell.

**Start the server + UI** (from the repo root):

```bash
source .venv/bin/activate            # if not already active
scripts/dev_application_ui.sh        # one command: backend :8765 + Vite :5173
```

Open <http://localhost:5173/>. The welcome screen shows example prompts you can
click to try it out. Press `Ctrl-C` in the terminal to stop both processes.

Prefer not to use `.env`? Anything you already exported in your shell wins
over `.env`, and the `oh` CLI reads the same variables, so existing `oh setup`
configurations work out of the box:

```bash
export OPENAI_API_KEY=...                           # required
export OPENAI_BASE_URL=https://api.moonshot.cn/v1   # e.g. Kimi; leave unset for OpenAI itself
export OPENHARNESS_MODEL=kimi-k2.5
```

Need different ports (e.g. `8765` already taken)? Override with env vars:

```bash
OH_WEB_PORT=8766 VITE_PORT=5176 scripts/dev_application_ui.sh
```

You can also point the UI at a remote backend at runtime via the **Settings**
dialog in the top-right — the URL you enter is saved to `localStorage`.
...
- [`docs/app/SHOWCASE.md`](SHOWCASE.md) for real-world usage patterns worth documenting
