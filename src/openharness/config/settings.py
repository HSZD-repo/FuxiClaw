...
class MemorySettings(BaseModel):
    ...
    max_recall_chars: int = 2500
    max_fixed_chars: int = 1200
...
def default_provider_profiles() -> dict[str, ProviderProfile]:
    ...
    return {
        ...
        "openai-compatible": ProviderProfile(
            ...
            allowed_models=["gpt-5.5", "gpt-5.4", "gpt-5", "gpt-4.1", "o4-mini"],
        ),
        ...
        "moonshot": ProviderProfile(
            ...
            allowed_models=["kimi-k2.5", "kimi-k2.6"],
        ),
        ...
        "dashscope": ProviderProfile(
            ...
            allowed_models=["qwen3.5-flash", "qwen3-max"],
        ),
        ...
        "mimo": ProviderProfile(
            ...
            provider="mimo",
            api_format="openai",
            auth_source="mimo_api_key",
            default_model="mimo-2.5",
            base_url=None,
            allowed_models=["mimo-2.5"],
        ),
    }
...
class Settings(BaseModel):
    ...
    def merged_profiles(self) -> dict[str, ProviderProfile]:
        ...
        for name, raw_profile in self.profiles.items():
            ...
            if (
                ...
                profile = builtin.model_copy(
                    update={
                        "last_model": profile.last_model,
                        "base_url": (
                            profile.base_url
                            if profile.base_url is not None
                            else builtin.base_url
                        ),
    ...
    def resolve_auth(self) -> ResolvedAuth:
        ...
        if explicit_key and auth_source in {"anthropic_api_key", "openai_api_key"}:
            return ResolvedAuth(
                provider=provider or storage_provider,
                auth_kind="api_key",
                value=explicit_key,
...
def _apply_env_overrides(settings: Settings) -> Settings:
    ...
    tool_execution_mode = os.environ.get("OPENHARNESS_TOOL_EXECUTION_MODE")
    if tool_execution_mode:
        updates["tool_execution_mode"] = tool_execution_mode.strip().lower()
