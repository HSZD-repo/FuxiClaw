...
class GlobTool(BaseTool):
    ...
    async def execute(self, arguments: GlobToolInput, context: ToolExecutionContext) -> ToolResult:
        ...
        matches = await _glob(context, root, arguments.pattern, limit=arguments.limit)
        if not matches:
...
async def _glob(context: ToolExecutionContext, root: Path, pattern: str, *, limit: int) -> list[str]:
    ...
    if rg and ("**" in pattern or "/" in pattern):
        ...
        await register_step_cancel_handle(
            context,
            lambda **_: _terminate_process(process),
            kind="subprocess",
            label="ripgrep files",
        )
