    ...
    def get_auth_source_statuses(self) -> dict[str, Any]:
        ...
        for source in _AUTH_SOURCES:
            ...
            if source == "anthropic_api_key":
                if load_credential(storage_provider, "api_key") or getattr(self.settings, "api_key", ""):
                    configured = True
                    origin = "file"
                    state = "configured"
                ...
                if load_credential(storage_provider, "api_key"):
                    configured = True
                    origin = "file"
                    state = "configured"
                ...
                if load_credential(storage_provider, "api_key"):
                    ...
                    env_name = _ENV_BY_AUTH_SOURCE.get(source)
        ...
        for provider in _KNOWN_PROVIDERS:
            ...
            if provider == "anthropic":
                ...
                if os.environ.get("DEEPSEEK_API_KEY"):
                    configured = True
                    source = "env"
                elif load_credential("deepseek", "api_key"):
                    configured = True
                    source = "file"
                ...
                if os.environ.get("ZHIPUAI_API_KEY"):
                    configured = True
                    source = "env"
                elif load_credential("zhipu", "api_key"):
                    configured = True
                    source = "file"
                ...
                if os.environ.get("MIMO_API_KEY"):
                    configured = True
                    source = "env"
                elif load_credential("mimo", "api_key"):
                    configured = True
                    source = "file"
    ...
    def get_profile_statuses(self) -> dict[str, Any]:
        ...
        for name, profile in self.list_profiles().items():
            ...
            statuses[name] = {
                ...
                "auth_origin": "settings" if source_status.get("source") == "file" else str(source_status.get("source", "missing")),
                "active": name == active,
